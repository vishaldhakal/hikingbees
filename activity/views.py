...
from .serializers import (
    ...
    ActivityCategorySerializerSmall,
    ActivityCategorySlugSerializer,
    ActivityCategorySmallSerializer,
    ActivityImageSerializer,
    ActivityRegionSerializer,
    ActivityRegionSlugSerializer,
    ActivityRegionSmallSerializer,
    ActivitySearchSerializers,
    ActivitySerializer,
    ActivitySlugSerializer,
    ActivitySmallSerializer,
    ActivityTestimonialSerializer,
    DestinationSerializer,
    DestinationSerializerSmall,
    DestinationSmallSerializer,
    ItineraryActivitySerializer,
    LandingActivityRegionSmallSerializer,
...
def sign_view(request):
    try:
        ...
        hash_obj = hmac.new(
            ...
            msg=force_bytes(field_values_joined),
...
def activities_slug(request):
    if request.method == "GET":
        activities = Activity.objects.only("id", "slug")
...
def activities_single(request, slug):
    if request.method == "GET":
        ...
        grouped_bookings = []
        booking_dates = (
            ActivityBooking.objects.filter(is_verified=True, is_private=False)
            .annotate(booking_date_date=Cast("booking_date", output_field=DateField()))
            .values("booking_date_date")
            .distinct()
        )
        ...
        for datee in unique_dates:
            ...
            boki = ActivityBooking.objects.filter(
...
def activity_pdf_detail(request, slug):
    ...
    altitude_chart_url = (
        activity.altitude_chart.url if activity.altitude_chart else None
    )
