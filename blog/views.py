...
def post_list(request):
    if request.method == 'GET':
        ...
        posts_serializer = LandingPagePostSerializer(
...
def post_single(request, slug):
    if request.method == 'GET':
        ...
        similar_posts = (Post.objects .filter(tags__in=posts.tags.all()) .exclude(id=posts.id) .annotate(
            shared_tags=models.Count('tags', filter=models.Q(tags__in=posts.tags.all()))) .order_by('-shared_tags', '-created_at'))[:5]
        serializer = PostSerializer(posts)
        similar_posts_serializer = NavbarPostSerializer(
            similar_posts, many=True)
